...
def getClosedWarmstart(ws_file, autosave=False, base_height=0.575, slope=0.0, velocity=0.2, comWeight=0):
    ...
    for t, pattern in tqdm(enumerate(contactPattern[:-1])):
        ...
        xreg_res = crocoddyl.ResidualModelState(state, xs_closed[t], actuation.nu)
        xreg_act = crocoddyl.ActivationModelWeightedQuad(weights_state)
        xreg_cost = crocoddyl.CostModelResidual(state, xreg_act, xreg_res)
        cost.addCost("xreg", xreg_cost, 10)
    ...
    xref_term_res = crocoddyl.ResidualModelState(state, xs_closed[-1], actuation.nu)
    ...
    term_cost.addCost("xreg", xref_term_cost, 10)
    ...
    problem = crocoddyl.ShootingProblem(xs_closed[0], runnings, terminal_model)
    ...
    solver.solve(maxiter=100, init_xs=[xs_closed[0] for t in range(T)], init_us=[us_open[0] for t in range(T-1)])

    if not autosave:
        while input("Press q to quit the visualisation") != "q" and not autosave:
            viz.play(np.array(solver.xs)[:, :nq_closed], walkParams.DT)
