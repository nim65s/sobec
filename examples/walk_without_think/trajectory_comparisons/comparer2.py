...
class Comparer:
    def __init__(self, slope_list=[], base_list=[600], velocity_list=[], comWeight_list=[]):
        self.logs = {}
        self.names = []
        self.robots = {}
        for b in base_list:
        ...
        self.walkParams = {"slope": {}, "velocity": {}, "comWeight": {}}
        for s in slope_list:
        ...
        for comWeight in comWeight_list:
            p_open = specific_params.WalkBattobotParams("open")
            p_open.comWeight = comWeight
            p_closed = specific_params.WalkBattobotParams("closed")
            p_closed.comWeight = comWeight
            self.walkParams["comWeight"][comWeight] = {"open": p_open, "closed": p_closed}
        ...
        self.ddps = {"slope": {}, "velocity": {}, "comWeight": {}}
        ...
        for key in self.walkParams["comWeight"].keys():
            self.ddps["comWeight"][key] = {
                "open": sobec.wwt.buildSolver(
                    robot_open, self.contactPattern, self.walkParams["comWeight"][key]["open"]
                ),
                "closed": sobec.wwt.buildSolver(
                    robot_closed, self.contactPattern, self.walkParams["comWeight"][key]["closed"]
                ),
            }
        ...
        self.linestyles = ["-", "--", "-.", ":"]
        self.figsize = (12, 6)
        self.timeHorizon = [0, len(self.contactPattern)]        
    ...
    def add_log(self, path, robot_type, name, s=None, b=None, v=None, w=None):
        ...
        if s is not None:
            ...
            params = self.walkParams["comWeight"][w][robot_type]
            ddp = self.ddps["comWeight"][w][robot_type]
        elif b is not None:
            params = self.walkParams["slope"][b][robot_type]
            ddp = self.ddps["slope"][b][robot_type]
        else:
            raise ValueError("You must specify either s, v or w")
    ...
    def plotTrajGaps(self, name):
        ...
        for i, pos in enumerate(pos_gaps):
            axis_pos.plot(pos[self.timeHorizon[0]:self.timeHorizon[1]], c=self.colors[i%len(self.colors)], linestyle=self.linestyles[i%len(self.linestyles)])
        for i, vel in enumerate(vel_gaps):
            axis_vel.plot(vel[self.timeHorizon[0]:self.timeHorizon[1]], c=self.colors[i%len(self.colors)], linestyle=self.linestyles[i%len(self.linestyles)])
    ...
    def plotConstraintsResidual(self, name):
        ...
        ax_pos_loop.plot(np.linalg.norm(loop_pos_res, axis=0)[self.timeHorizon[0]:self.timeHorizon[1]])
        ax_vel_loop.plot(np.linalg.norm(loop_vel_res, axis=0)[self.timeHorizon[0]:self.timeHorizon[1]])
        ax_pos_floor.plot(np.linalg.norm(floor_pos_res, axis=0)[self.timeHorizon[0]:self.timeHorizon[1]])
        ax_vel_floor.plot(np.linalg.norm(floor_vel_res, axis=0)[self.timeHorizon[0]:self.timeHorizon[1]])
    ...
    def compareTotalCosts(self, names=[], ids=[], title=None):
        ...
        for i, name in enumerate(names):
            ...
            ax.plot(
                total_running_costs[self.timeHorizon[0]:self.timeHorizon[1]],
                label=name,
    ...
    def compareRunningCosts(self, names=[], ids=[], title=None, keys=[]):
        ...
        for i, name in enumerate(names):
            ...
            for j, (key, ax) in enumerate(zip(keys, axs.flatten())):
                ...
                ax.plot(
                    cost[self.timeHorizon[0]:self.timeHorizon[1]],
                    label=f"{name}", 
                    c=self.colors[i%len(self.colors)], 
                    linestyle=self.linestyles[i%len(self.linestyles)]
                )
    ...
    def plotControls(self, names=[], ids=[], title=None):
        ...
        for i, name in enumerate(names):
            ...
            for j, (us_i, ax) in enumerate(zip(us, axs.T.flatten())):
                ax.plot(
                    us_i[self.timeHorizon[0]:self.timeHorizon[1]],
                    label=name,
                    c=self.colors[i % len(self.colors)],
                    linestyle=self.linestyles[i % len(self.linestyles)],
                )
    ...
    def plotControlsDiff(self, names=[], ids=[], title=None, labels=[]):
        ...
        for i, name_tuple in enumerate(names):
            ...
            for j, (us_diff_i, ax) in enumerate(zip(us_diff, axs.T.flatten())):
                ax.plot(us_diff_i[self.timeHorizon[0]:self.timeHorizon[1]], label=labels[i], linestyle=self.linestyles[i%len(self.linestyles)])
                ax.set_title(f"u_{j}")
                ax.grid()
                ax.legend()
        plt.suptitle(title)
        ...
        for traj_type in ["position", "velocity"]:
            ...
            for i, name in enumerate(names):
                ...
                for j, (traj_ax, ax) in enumerate(zip(traj.T, axs.T.flatten())):
                    ax.plot(traj_ax[self.timeHorizon[0]:self.timeHorizon[1]], label=name, c=self.colors[i%len(self.colors)], linestyle=self.linestyles[i%len(self.linestyles)])
        ...
        for traj_type in ["position", "velocity"]:
            ...
            for i, name in enumerate(names):
                ...
                for j, (traj_xi, traj_yi, ax) in enumerate(zip(traj_x.T, traj_y.T, axs.T.flatten())):
                    ax.plot(traj_xi[self.timeHorizon[0]:self.timeHorizon[1]], traj_yi[self.timeHorizon[0]:self.timeHorizon[1]], label=name, c=self.colors[i%len(self.colors)], linestyle=self.linestyles[i%len(self.linestyles)])
                    ax.set_xlabel("x")
                    ax.set_ylabel("y")
    ...
    def plotCOMXYTrajectory(self, names=[], ids=[], title=None, velocity=True):
        ...
        for traj_type in traj_types:
            ...
            for i, name in enumerate(names):
                ...
                traj_x = traj[:, 0]
                traj_y = traj[:, 1]
                ax.plot(traj_x[self.timeHorizon[0]:self.timeHorizon[1]], traj_y[self.timeHorizon[0]:self.timeHorizon[1]], label=name, c=self.colors[i%len(self.colors)], linestyle=self.linestyles[i%len(self.linestyles)])
                ax.set_xlabel("x")
                ax.set_ylabel("y")
        ...
        for traj_type in traj_types:
            ...
            for i, name_tuple in enumerate(names):
                ...
                for j, (com_diff, ax) in enumerate(zip(com_diff.T, axs.T.flatten())):
                    ax.plot(com_diff[self.timeHorizon[0]:self.timeHorizon[1]], label=labels[i], linestyle=self.linestyles[i%len(self.linestyles)])
    ...
    def plotCOMXYDiff(self, names=[], ids=[], title=None):
        ...
        for traj_type in ["position", "velocity"]:
            ...
            for i, name_tuple in enumerate(names):
                ...
                ax.plot(traj_x[self.timeHorizon[0]:self.timeHorizon[1]], traj_y[self.timeHorizon[0]:self.timeHorizon[1]], label=f"Diff {name_tuple[0]} / {name_tuple[1]}", c=self.colors[i%len(self.colors)], linestyle=self.linestyles[i%len(self.linestyles)])
                ax.set_xlabel("x")
                ax.set_ylabel("y")
                ax.set_title(f"COM {traj_type} - XY plane")
                ax.grid()
